...
function checkLineContent(line){
    ...
    if(sourceObj !== undefined && line.indexOf("@")<0 && sourceObj.type !== "class"){
        const lineBreaks = line.split(":");
        sourceObj.props[lineBreaks[0].trim()] = lineBreaks[1].replace(",", "").replaceAll("'", "").trim();
    }

    // class
    if(line.indexOf("export class")==0 || line.indexOf("class")==0){
        sourceObj = finalObject;
    ...
    // add class props
    if(sourceObj !== undefined && sourceObj.type === "class" && ((line.indexOf("export class")!==0 && line.indexOf("class")!==0))){
